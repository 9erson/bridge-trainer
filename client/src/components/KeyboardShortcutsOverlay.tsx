...
const openingBidShortcuts: ShortcutGroup = {
  ...
  shortcuts: [
    ...
    { keys: '1C, 1D, 1H, 1S, 1N', description: '1-level bids (type level then strain)' },
...
export default function KeyboardShortcutsOverlay({ isOpen, onClose, context }: Props) {
  useEffect(() => {
    if (!isOpen) return;
          ...
          >
            ...
            <div className="p-3 border-t border-border text-center">
              <span className="text-xs text-muted-foreground">
                Press <kbd className="px-1.5 py-0.5 bg-muted border border-border rounded text-xs font-mono">?</kbd> or <kbd className="px-1.5 py-0.5 bg-muted border border-border rounded text-xs font-mono">Esc</kbd> to close
              </span>
            </div>
          </motion.div>
        </motion.div>
