...
): { hand: BridgeHand; bid: string; description: string; seat: SeatPosition; vuln: Vulnerability } {
...
export default function OpeningBidPlay({ settings, onComplete, onQuit }: Props) {
  ...
  const generateNewHand = useCallback(() => {
    const newData = generateHandForBidding(conventionId, settings.difficulty, availableBids);
    setHandData(newData);
    setFeedback(null);
    setSelectedIndex(0);
  ...
  const submitAnswer = useCallback(
    (userBid: string) => {
      ...
      const handResult: HandResultData = {
        ...
        handData: { ...handData.hand, seat: handData.seat, vuln: handData.vuln },
        userAnswer: userBid,
  ...
  }, [feedback, availableBids, selectedIndex, sequenceBuffer, submitAnswer, handleNext]);
  ...
  return (
    ...
    >
              ...
              {settings.difficulty !== 'easy' && (
                <>
                  ...
                  {settings.difficulty === 'hard' && (
                    <span className="bg-muted px-2 py-0.5 rounded font-medium">
                      Vuln: {handData.vuln === 'none' ? 'None' : handData.vuln.toUpperCase()}
                    </span>
                  )}
                </>
              )}
          ...
          {sequenceBuffer && (
            <div className="text-center mb-2">
              <span className="inline-flex items-center px-2 py-0.5 bg-primary/10 border border-primary/30 rounded text-xs font-mono text-primary">
                {sequenceBuffer}_ <span className="ml-1 text-muted-foreground">(type C/D/H/S/N)</span>
              </span>
            </div>
          )}
      ...
      <InlineReference conventionId={conventionId} isOpen={showReference} onToggle={() => setShowReference((p) => !p)} />
    </GameShell>
  );
}
