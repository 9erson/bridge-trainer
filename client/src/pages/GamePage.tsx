...
import { getGame, type GameSettings, type GameResults } from '@/lib/gameRegistry';
...
export default function GamePage() {
  ...
  const shortcutContext = params.gameId === 'point-counting' ? 'point-counting' as const
    : params.gameId === 'opening-bid' ? 'opening-bid' as const
    : 'general' as const;
  ...
  return (
    <div>
      ...
      {phase === 'play' && settings && (
        <PlayComponent settings={settings} onComplete={handleComplete} onQuit={handleQuit} />
      )}
      {phase === 'results' && results && (
        <ResultsComponent results={results} onPlayAgain={handlePlayAgain} onBackToMenu={handleBackToMenu} />
      )}
      ...
      {phase !== 'setup' && (
        <div className="mt-6 text-center">
          <span className="text-[10px] text-muted-foreground/50">
            Press <kbd className="px-1 py-0.5 bg-muted/50 border border-border/50 rounded text-[9px] font-mono">?</kbd> for keyboard shortcuts
          </span>
        </div>
      )}
    </div>
