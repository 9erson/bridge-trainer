...
export interface RuleCondition {
  type: 'hcp_range' | 'balanced' | 'semi_balanced' | 'suit_length' | 'suit_length_min' | 'longest_suit' | 'not_balanced' | 'hcp_min' | 'any_suit_length_min' | 'no_5card_major' | 'has_5card_major' | 'no_4card_major' | 'has_4card_major' | 'longer_minor' | 'equal_minors_prefer';
...
function evaluateCondition(hand: BridgeHand, condition: RuleCondition, _seat: SeatPosition, _vuln: Vulnerability): boolean {
...
export function evaluateOpeningBid(
  ...
  for (const rule of convention.rules) {
    const allMatch = rule.conditions.every((cond) =>
      evaluateCondition(hand, cond, seat, vulnerability)
    );
    if (allMatch) {
  ...
  return { bid: 'Pass', description: 'Does not meet any opening bid requirements' };
}
...
export function getCorrectBid(
  ...
  const result = evaluateOpeningBid(hand, conventionId, seat, vulnerability);
...
export function getBidsForDifficulty(difficulty: string): string[] {
  switch (difficulty) {
    ...
    case 'medium':
      return ['Pass', '1C', '1D', '1H', '1S', '1NT', '2C', '2D', '2H', '2S', '2NT'];
    case 'hard':
      return ['Pass', '1C', '1D', '1H', '1S', '1NT', '2C', '2D', '2H', '2S', '2NT', '3C', '3D', '3H', '3S'];
    default:
      return ['Pass', '1C', '1D', '1H', '1S', '1NT'];
  }
}
